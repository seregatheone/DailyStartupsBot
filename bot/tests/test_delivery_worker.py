...
class DeliveryWorkerTest(unittest.TestCase):
    ...
    def test_reports_one_success_after_all_delivery_messages(self) -> None:
        backend = FakeBackend(
            [
                {
                    "id": "delivery-1",
                    "telegram_id": 42,
                    "messages": [
                        {"text": "Digest part 1", "parse_as": "HTML"},
                        {"text": "Digest part 2", "parse_as": "HTML"},
                    ],
                }
            ]
        )
        ...
        self.assertEqual(backend.attempts[0][1]["status"], "success")
        self.assertEqual(backend.attempts[0][1]["telegram_message_id"], "101")
