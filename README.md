...
- `DAILY_STARTUPS_SCHEDULED_E2E_TIMEOUT_SECONDS` (default: `180`)
- `DAILY_STARTUPS_SCHEDULED_E2E_RECEIPT_PATH` (default: `.runtime/daily-startups/scheduled-telegram-e2e-receipt.json`)
...
- `max` — суммарное количество стартапов от 5 до 10; default — 10.
...
Дайджест сначала выбирает до двух лучших уникальных записей от каждого продуктивного источника, затем заполняет свободные места по общему quality/recency ranking. Failed, `zero_yield` и исчерпанные источники не резервируют пустые места. Если валидных уникальных стартапов меньше пяти, бот показывает фактическое количество без synthetic items.
...
Для отдельной проверки scheduled ingestion → digest → queue → реальный Telegram send запустите opt-in сценарий:

```bash
export DAILY_STARTUPS_TELEGRAM_TOKEN='replace-with-test-token'
export DAILY_STARTUPS_E2E_TELEGRAM_ID='replace-with-dedicated-test-chat-id'
make scheduled-telegram-e2e
```

Runner поднимает backend на случайном loopback-порту с новой временной SQLite, включает утверждённые live sources и запускает только delivery worker без Telegram long polling. Он проверяет terminal `sent`, Telegram acknowledgement, общий предел 10, последовательные ranks и publisher attribution каждого выбранного item; затем останавливает процессы и удаляет временную базу. Token и Telegram ID не попадают в machine-readable receipt. Отсутствие credentials, сети или test recipient завершает прогон с FAIL, а не с ложным PASS.
...
Проверьте формат `ключ=значение`, IANA timezone и диапазон `max=5..10`. Пример:
